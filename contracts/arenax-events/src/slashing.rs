...
pub fn emit_escrow_contract_set(env: &Env, escrow_contract: &Address) {
    EscrowContractSet { escrow_contract: escrow_contract.clone() }.publish(env);
}

pub fn emit_case_opened(env: &Env, case_id: &BytesN<32>, subject: &Address, initiator: &Address, reason_code: u32, evidence_hash: &BytesN<32>) {
    CaseOpened { case_id: case_id.clone(), subject: subject.clone(), initiator: initiator.clone(), reason_code, evidence_hash: evidence_hash.clone() }.publish(env);
}

pub fn emit_case_approved(env: &Env, case_id: &BytesN<32>, approver: &Address) {
    CaseApproved { case_id: case_id.clone(), approver: approver.clone() }.publish(env);
}

pub fn emit_penalty_executed(env: &Env, case_id: &BytesN<32>, penalty_type: u32, subject: &Address) {
    PenaltyExecuted { case_id: case_id.clone(), penalty_type, subject: subject.clone() }.publish(env);
}

pub fn emit_case_cancelled(env: &Env, case_id: &BytesN<32>, subject: &Address) {
    CaseCancelled { case_id: case_id.clone(), subject: subject.clone() }.publish(env);
}

pub fn emit_stake_slashed(env: &Env, subject: &Address, amount: i128, asset: &Address) {
    StakeSlashed { subject: subject.clone(), amount, asset: asset.clone() }.publish(env);
}

pub fn emit_reward_confiscated(env: &Env, subject: &Address, amount: i128, asset: &Address) {
    RewardConfiscated { subject: subject.clone(), amount, asset: asset.clone() }.publish(env);
}

pub fn emit_temporary_suspension(env: &Env, subject: &Address, duration: u64, expires_at: u64) {
    TemporarySuspension { subject: subject.clone(), duration, expires_at }.publish(env);
}

pub fn emit_permanent_ban(env: &Env, subject: &Address) {
    PermanentBan { subject: subject.clone() }.publish(env);
}
