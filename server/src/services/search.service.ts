...
class ElasticsearchBackend implements ISearchBackend {
    ...
    private readonly indexName = 'arenax';
    ...
    async index(doc: SearchDocument): Promise<void> {
        await fetch(`${this.baseUrl}/${this.indexName}/_doc/${this.esId(doc)}`, {
            method: 'PUT',
    ...
    async bulkIndex(docs: SearchDocument[]): Promise<void> {
        ...
        const lines = docs.flatMap((doc) => [
            JSON.stringify({ index: { _index: this.indexName, _id: this.esId(doc) } }),
            JSON.stringify(doc),
        ]);
    ...
    async delete(id: string, entity: SearchableEntity): Promise<void> {
        await fetch(`${this.baseUrl}/${this.indexName}/_doc/${entity}_${id}`, {
            method: 'DELETE',
        });
    }
