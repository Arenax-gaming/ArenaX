...
class ElasticsearchBackend implements ISearchBackend {
    ...
    private readonly esIndex = 'arenax';
    ...
    async index(doc: SearchDocument): Promise<void> {
        await fetch(`${this.baseUrl}/${this.esIndex}/_doc/${this.esId(doc)}`, {
            method: 'PUT',
    ...
    async bulkIndex(docs: SearchDocument[]): Promise<void> {
        ...
        const lines = docs.flatMap((doc) => [
            JSON.stringify({ index: { _index: this.esIndex, _id: this.esId(doc) } }),
            JSON.stringify(doc),
        ]);
    ...
    async delete(id: string, entity: SearchableEntity): Promise<void> {
        await fetch(`${this.baseUrl}/${this.esIndex}/_doc/${entity}_${id}`, {
            method: 'DELETE',
        });
    }
    ...
    async search(opts: SearchOptions): Promise<SearchResult> {
        ...
        const res = await fetch(`${this.baseUrl}/${this.esIndex}/_search`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
