...
use backend::auth::{
    Claims, JwtConfig, JwtError, JwtService, SessionInfo, TokenType,
};
...
async fn test_token_validation_without_redis() {
    ...
    assert_eq!(access_token_parts.len(), 3, "Access token should have 3 parts");
    assert_eq!(refresh_token_parts.len(), 3, "Refresh token should have 3 parts");
}
...
async fn test_claims_structure() {
    ...
    assert!(refresh_decoded.is_ok(), "Should be able to decode refresh token");
...
async fn test_key_rotation_basic() {
    ...
    assert!(!jwt_service.should_rotate_keys(), "Should not need rotation immediately after rotation");
}
...
async fn test_security_policies() {
    ...
    assert!(policy_result.is_ok(), "Valid claims should pass security policies");
    ...
    assert!(policy_result.is_err(), "Claims with session too long should fail");
    ...
    assert!(policy_result.is_err(), "Claims with too many refreshes should fail");
...
async fn test_token_analytics_structure() {
    ...
    assert!(age < Duration::seconds(1), "Analytics should be recently updated");
}
...
async fn test_session_info_structure() {
    ...
    assert!(serialized.is_ok(), "Should be able to serialize SessionInfo");

    let deserialized: Result<SessionInfo, _> = serde_json::from_str(&serialized.unwrap());
    assert!(deserialized.is_ok(), "Should be able to deserialize SessionInfo");
...
async fn test_token_types() {
    ...
    let access_deserialized: Result<TokenType, _> = serde_json::from_str(&access_serialized.unwrap());
    let refresh_deserialized: Result<TokenType, _> = serde_json::from_str(&refresh_serialized.unwrap());
...
async fn test_performance_token_generation() {
    ...
    assert!(tokens_per_second > 50.0, "Token generation should be fast enough");
    ...
    assert!(analytics.total_tokens_issued >= num_tokens,
        "Should have generated at least {} tokens, got {}",
        num_tokens, analytics.total_tokens_issued);
}
