...
/// Authentication Service with JWT and Redis-backed session management
#[derive(Clone)]
...
impl AuthService {
    pub fn new(pool: DbPool, jwt_service: Arc<JwtService>) -> Self {
    ...
    pub async fn register(&self, request: CreateUserRequest) -> Result<AuthResponse, ApiError> {
        ...
        let token_pair = self
            .jwt_service
            .generate_token_pair(user.id, vec!["user".to_string()], None)
            .await
    ...
    /// Logout user (blacklist token, revoke session)
...
mod tests {
    ...
    fn test_password_length_validation() {
        assert!("short".len() < 8);
        assert!("long_enough_password".len() >= 8);
    }
}
